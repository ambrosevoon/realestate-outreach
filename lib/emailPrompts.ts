...
export const EMAIL_PROMPTS_TABLE = 're_outreach_prompt_versions'

export const DEFAULT_EMAIL_PROMPT_NAME = 'Live Prompt Baseline'

export const DEFAULT_EMAIL_PROMPT_USER_TEMPLATE = `Write a fresh outreach email variation [SEED] for:
- Name: [NAME]
- Agency: [AGENCY]
- Suburb: [SUBURB]
...
export const DEFAULT_EMAIL_PROMPT_SYSTEM = `You are generating outbound cold outreach email copy for a real estate agent outreach system.
...
export function buildFallbackPromptVersion(): EmailPromptVersion {
  const now = new Date().toISOString()
  return {
    id: 'built-in-default-prompt',
    name: DEFAULT_EMAIL_PROMPT_NAME,
    user_prompt_template: DEFAULT_EMAIL_PROMPT_USER_TEMPLATE,
    system_prompt: DEFAULT_EMAIL_PROMPT_SYSTEM,
    is_active: true,
    created_at: now,
    updated_at: now,
    activated_at: now,
...
export function buildPromptVersionName(date = new Date()) {
  return `Prompt ${date.toLocaleString('en-AU', {
    year: 'numeric',
    month: 'short',
    day: '2-digit',
    hour: '2-digit',
...
export function renderPromptTemplate(
  ...
  const customBlock = customInstructions ? `Custom instructions: ${customInstructions}` : ''

  return template
    .replaceAll('[SEED]', values.seed)
    .replaceAll('[NAME]', values.name)
    .replaceAll('[AGENCY]', values.agency_name)
    .replaceAll('[SUBURB]', values.suburb || '')
    .replaceAll('[CUSTOM_INSTRUCTIONS_BLOCK]', customBlock)
    .replaceAll('[CUSTOM_INSTRUCTIONS]', customInstructions)
    .replace(/\n{3,}/g, '\n\n')
    .trim()
}
