  ...
  const modal = (
    ...
    >
      <div
        ...
        className="relative flex max-h-[90vh] w-full max-w-3xl flex-col overflow-hidden rounded-[2rem] border border-white/10 bg-[linear-gradient(180deg,rgba(7,11,18,0.98),rgba(5,8,14,0.98))] shadow-2xl shadow-black/60"
      >
        {/* Header */}
        <div className="border-b border-white/8 px-6 pb-4 pt-5 flex-shrink-0 bg-[radial-gradient(circle_at_top_right,rgba(212,164,97,0.12),transparent_34%)]">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0 flex-1">
              ...
              <div className="flex items-center gap-1.5 mt-1">
                ...
                <span className="text-sm text-stone-300 truncate">{lead.agency_name}</span>
              </div>
            </div>
            <div className="flex items-center gap-2 flex-shrink-0 mt-0.5">
              ...
              <button
                onClick={onClose}
                className="flex h-7 w-7 items-center justify-center rounded-lg text-stone-500 transition-colors hover:bg-white/8 hover:text-white cursor-pointer"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
        ...
        <div className="overflow-y-auto flex-1 px-6 py-4 space-y-5">
          ...
          {/* Status */}
          <div className="space-y-2">
            <label className="text-xs font-medium text-stone-400 uppercase tracking-[0.22em]">
              Status
            </label>
            <div className="flex items-center gap-2">
              <Select value={status} onValueChange={v => saveStatus(v as LeadStatus)}>
                <SelectTrigger className="flex-1 border-white/10 bg-slate-950/60 text-white focus:ring-amber-500">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent className="border-white/10 bg-slate-900/95">
          ...
          <div className="space-y-2">
            ...
            <Textarea
              ...
              rows={3}
              className="resize-none border-white/10 bg-slate-950/60 text-sm text-white placeholder:text-stone-500 focus-visible:ring-amber-500"
            />
            <Button
              size="sm"
              variant="outline"
